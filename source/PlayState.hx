...
class PlayState extends MusicBeatState
{
	...
	public static var ratingStuff:Array<Dynamic> = [
		...
		['Sunglasses emoji', 1] //The value on this one isn't used actually, since Perfect is always "1"
	];
	...
	public var darkLevels:Array<String> = ['banbodi-but-sexo', 'el-bambox'];
	...
	var wavyThingy:Shaders.GlitchEffect;
	...
	public var funnyFloatyBoys:Array<String> = ['banbodi', 'bambox'];
	...
	public var adultSwim:Bool = false;
	...
	override public function create()
	{
		...
		wavyThingy = new Shaders.GlitchEffect();
		wavyThingy.waveAmplitude = 0.1;
		wavyThingy.waveFrequency = 1;
		wavyThingy.waveSpeed = 2;
		...
		switch (curStage)
		{
			...
			case 'banbodi-but-sexo':
				var banbodi:FlxSprite = new FlxSprite(-400, -100).loadGraphic(Paths.image('banbodi/detroyer_background'));
				banbodi.scale.set(3, 3);
				banbodi.shader = wavyThingy.shader;
				add(banbodi);

			case 'el-bambox':
				var bambox:FlxSprite = new FlxSprite(-400, -100).loadGraphic(Paths.image('banbodi/bmbox_Background'));
				bambox.scale.set(3, 3);
				bambox.shader = wavyThingy.shader;
				add(bambox);			
		...
		if(SONG.song.toLowerCase() == 'inscription') {
			swagger = new Character(120, -945, 'banbodi');			
			startCharacterPos(swagger, true);
			dadGroup.add(swagger);
		}
		...
		sicks2 = new FlxText(0, 350, 0, "", 22);
		sicks2.setFormat(Paths.font("comic.ttf"), 22, FlxColor.WHITE, FlxTextAlign.LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		sicks2.borderSize = 1.25;
		sicks2.scrollFactor.set();
		sicks2.cameras = [camHUD];
		sicks2.text = 'Sicks: ${sicks}';
		add(sicks2);
		
		goods2 = new FlxText(0, 370, 0, "", 22);
		goods2.setFormat(Paths.font("comic.ttf"), 22, FlxColor.WHITE, FlxTextAlign.LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		goods2.borderSize = 1.25;
		goods2.scrollFactor.set();
		goods2.cameras = [camHUD];
		goods2.text = 'Goods: ${goods}';
		add(goods2);

		bads2 = new FlxText(0, 390, 0, "", 22);
		bads2.setFormat(Paths.font("comic.ttf"), 22, FlxColor.WHITE, FlxTextAlign.LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		bads2.borderSize = 1.25;
		bads2.scrollFactor.set();
		bads2.cameras = [camHUD];
		bads2.text = 'Bads: ${bads}';
		add(bads2);

		shits2 = new FlxText(0, 410, 0, "", 22);
		shits2.setFormat(Paths.font("comic.ttf"), 22, FlxColor.WHITE, FlxTextAlign.LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		shits2.borderSize = 1.25;
		shits2.scrollFactor.set();
		shits2.cameras = [camHUD];
		shits2.text = 'Shits: ${shits}';
		add(shits2);

		misses = new FlxText(0, 430, 0, "", 22);
		misses.setFormat(Paths.font("comic.ttf"), 22, FlxColor.WHITE, FlxTextAlign.LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		songWatermark = new FlxText(0, textYPos, 0, "", 16);
		songWatermark.text = SONG.song + " " + (storyDifficulty == 2 ? "Hard" : storyDifficulty == 1 ? "Nomral" : "Easy") + " | " + 'Socks Engine' + " 0.1 " + "(PE 0.4.2)";
		...
		for (section in noteData)
		{
			for (songNotes in section.sectionNotes)
			{
				if(songNotes[1] > -1) { //Real notes
					...
					if (((Note.CharactersWith3D.contains(PlayState.SONG.player2) && !gottaHitNote)) || ((Note.CharactersWith3D.contains(PlayState.SONG.player2)) && ((swagNote.strumTime / 50) % 20 > 10)))
					{
						swagNote.texture = 'NOTE_assets_3D';
					} else {
					...
					if(floorSus > 0) {
						for (susNote in 0...floorSus+1)
						{
							...
							if (((Note.CharactersWith3D.contains(PlayState.SONG.player2) && !gottaHitNote)) || ((Note.CharactersWith3D.contains(PlayState.SONG.player2)) && ((sustainNote.strumTime / 50) % 20 > 10)))
							{
								sustainNote.texture = 'NOTE_assets_3D';
							} else {
			...
			switch(SONG.song.toLowerCase()) {
				case 'inscription':
					switch(curBeat) {
					    case 319: 
							adultSwim = true;
							FlxTween.tween(swagger, {y: swagger.y + 950}, 1, {ease:FlxEase.cubeInOut});
							FlxTween.tween(dad, {x: dad.x - 450}, 1, {ease:FlxEase.cubeInOut});
					}
			}
				...
				if (!daNote.mustPress && daNote.wasGoodHit && !daNote.hitByOpponent && !daNote.ignoreNote)
				{
					...
					if(daNote.noteType == 'Hey!' && dad.animOffsets.exists('hey')) {
						...
						if(daNote.noteType == 'GF Sing') {
							...
							gf.holdTimer = 0;
						} else if(daNote.noteType != 'Bambi Sing') {
							dad.playAnim(animToPlay + altAnim, true);
							dad.holdTimer = 0;
							adultSwim = false;
						}
						if(SONG.song.toLowerCase() == 'inscription') {
							if(daNote.noteType == 'Bambi Sing') {
								swagger.playAnim(animToPlay + altAnim, true);
								swagger.holdTimer = 0;	
								adultSwim = true;	
							}
						}
	...
	function moveCameraSection(?id:Int = 0, isNote:Bool = false):Void {
		...
		if (swagger != null && adultSwim)
		{
			var yOffsetB:Int = 0;
			var xOffsetB:Int = 0;
			if (followChars){
				if (swagger.animation.curAnim.name.startsWith('singUP')){
					yOffsetB = -25;
					xOffsetB = 0;
				}
				else if (swagger.animation.curAnim.name.startsWith('singDOWN')){
					yOffsetB = 25;
					xOffsetB = 0;
				}
				else if (swagger.animation.curAnim.name.startsWith('singLEFT')){
					yOffsetB = 0;
					xOffsetB = -25;
				}
				else if (swagger.animation.curAnim.name.startsWith('singRIGHT')){
					yOffsetB = 0;
					xOffsetB = 25;
				}
				else if (!swagger.animation.curAnim.name.startsWith('sing')){
					yOffsetB = 0;
					xOffsetB = 0;
				}
			}
			...
			camFollow.set(swagger.getMidpoint().x + xOffsetB, swagger.getMidpoint().y + yOffsetB);
			camFollow.x += swagger.cameraPosition[0] + stageData.opponent[0];
			camFollow.y += swagger.cameraPosition[1] + stageData.opponent[1];
			tweenCamIn();
			callOnLuas('onMoveCamera', ['swagger']);
			return;
		}
	...
	override function beatHit()
	{
		...
		if (curBeat % gfSpeed == 0) {
			curBeat % (gfSpeed * 2) == 0 ? {
				...
				iconP2.scale.set(1.1, 1.4);
			} : {
				iconP1.scale.set(1.1, 1.4);
